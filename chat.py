    ...
    if model_toggle == "Llama3":
        ...
        if selected == "對話":
            ...
            if api_key_input != st.session_state['chatbot_api_key']:
                ...
                st.experimental_rerun()
            st.session_state['open_ai_model'] = st.selectbox("選擇 ChatGPT 模型", ["gpt-3.5-turbo", "gpt-4"], index=["gpt-3.5-turbo", "gpt-4"].index(st.session_state.get('open_ai_model', 'gpt-3.5-turbo')))
